...
export const create_post = async (req: Request, res: Response) => {
  ...
  const returnedPost = await prisma.posts.findUnique({
    where: { Id: newPost.Id },
    select: {
      Id: true,
      Text: true,
  ...
  await client.del(`/posts/${req.params.UserId}`);
...
export const get_timeline_posts = async (req: Request, res: Response) => {
  ...
  const postsI = await prisma.posts.findMany({
    where: {
      OR: [{ UserId: req.params.UserId }, { UserId: { in: friendsIds } }],
    },
    select: {
      Id: true,
      Text: true,
...
export const delete_post = async (req: Request, res: Response) => {
  ...
  await client.del(`/posts/${deleted.UserId}`);
...
export const edit_post = async (req: Request, res: Response) => {
  ...
  const schema = Joi.object({
    Text: Joi.string().min(1).max(5000).required(),
    deleteImage: Joi.string().min(1).max(10).required(),
  });
  ...
  try {
    ...
    if (file)
      updatedPost = await prisma.posts.update({
        where: { Id: req.params.PostId },
        data: { Text: req.body.Text, Image: file.location },
      });
    else
      updatedPost = await prisma.posts.update({
        where: { Id: req.params.PostId },
    ...
    await client.del(`/posts/${returnedPost?.Users?.Id}`);
...
export const get_pagninated_posts = async (req: Request, res: Response) => {
  ...
  try {
    ...
    //filters out Ids of friends and into an array
    const friendsIds: Array<any> = friendships.map((friendship) =>
      friendship.User1 === req.params.UserId
        ? friendship.User2
        : friendship.User1
    );
